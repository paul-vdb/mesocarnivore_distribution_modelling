...
cameras_columbian_unique <-  cameras_columbian %>% distinct(MID_3km, .keep_all = TRUE)
DNA_columbian <- st_intersection(columbian_area, DNA_grid) # reducing smapling sites to grid cell of 12km
DNA_columbian_unique <-  DNA_columbian %>% distinct(MID_3km, .keep_all = TRUE) # reducing smapling sites to grid cell of 12km
sites_columbian <- bind_rows(cameras_columbian_unique, DNA_columbian_unique)

plot1 = ggplot() +
  #geom_sf(data = grid_sf, fill= NA)+
  geom_sf(data = columbian_area, color= 'green')+
  geom_sf(data = DNA_columbian_unique, color= 'red')+
  #geom_sf(data = cameras_grid, color= 'purple')+
  # geom_sf(data = cameras_columbian, color= 'black') +
  geom_sf(data = cameras_columbian_unique, color= 'purple')#+
#geom_sf(data = Ssites_columbian_unique, color= 'red')
plot1

#3. Extract coordinates in Albers 

Ssites_columbian_albers<-  do.call(rbind, st_geometry(sites_columbian)) %>%  as_tibble()
Ssites_columbian2 <- cbind(st_drop_geometry(sites_columbian), Ssites_columbian_albers)
#Ssites_columbian2 <- rename(Ssites_columbian2, c(Long= V1, Lat= V2))coord.scale
coord.scale <- 1000
traps.scale <- as.data.frame(Ssites_columbian_albers/coord.scale)

#4. Simulate data for columbian region at 12km scale ####

# NEED TO SCALE COORDINATES TO REDUCE THE AREA IN THE STATE SPACE, plot X.s and Xo to make sure traps have the same scale ## 

#define statespace, using columbian subpopulation data 
...
#sampling 
p0.s<-0.3 #detection probability SCR
...
cam_sites
#J.s <- 25
J.s<-1045 # placing 1045 hair traps 3km grids
#J.o <- 50
J.o<-349 # placing 349 camera traps on 3km 

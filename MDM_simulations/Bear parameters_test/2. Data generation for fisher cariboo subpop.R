###################
##Data generation for cariboo subpopulations
###################
library(rmapshaper)
library(terra)
library(ggplot2)
...
grid_columbian_dec_sf <-  sf::st_as_sf(grid_columbian_dec)
...
#2. Filter by population, cariboo ####
setwd("C:/LocalR")
...
cameras_cariboo <- st_intersection(cariboo, Camera_data_sf_albers) # no intersection 

academics_cariboo <-  st_intersection(cariboo, MDM_academics_sf_albers)

DNA_cariboo <- st_intersection(cariboo, DNA_data_sf_albers)

sites_cariboo <- bind_rows(academics_cariboo, DNA_cariboo) # no data on cameras
...
#J.s <- 25
J.s<-1761 # placing 1761 hair traps on a grid of 

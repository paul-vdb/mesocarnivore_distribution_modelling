...
cameras_omineca <- st_intersection(omineca, sites_cam) # no intersection 
cameras_omineca_unique <-  cameras_omineca %>% distinct(MID_3km, .keep_all = TRUE)
DNA_omineca <- st_intersection(omineca, DNA_grid) # reducing smapling sites to grid cell of 12km
DNA_omineca_unique <-  DNA_omineca %>% distinct(MID_3km, .keep_all = TRUE) # reducing smapling sites to grid cell of 12km
DNA_omineca_unique$Habitat_type <- as.character(DNA_omineca_unique$Habitat_type)
sites_omineca <- bind_rows(cameras_omineca_unique, DNA_omineca_unique)

plot1 = ggplot() +
  geom_sf(data= omineca)+
  #geom_sf(data = grid_sf, fill= NA)+
  # geom_sf(data = DNA_omineca, color= 'green')+
  geom_sf(data = DNA_omineca_unique, color= 'red')+
  #geom_sf(data = cameras_grid, color= 'purple')+
  # geom_sf(data = cameras_omineca, color= 'black') +
  geom_sf(data = cameras_omineca_unique, color= 'purple')#+
#geom_sf(data = Ssites_omineca_unique, color= 'red')
plot1

#3. Extract coordinates in Albers 

Ssites_omineca_albers<-  do.call(rbind, st_geometry(sites_omineca)) %>%  as_tibble()
Ssites_omineca2 <- cbind(st_drop_geometry(sites_omineca), Ssites_omineca_albers)
#Ssites_omineca2 <- rename(Ssites_omineca2, c(Long= V1, Lat= V2))coord.scale
coord.scale <- 1000
traps.scale <- as.data.frame(Ssites_omineca_albers/coord.scale)

#4. Simulate data for omineca region at 12km scale ####

# NEED TO SCALE COORDINATES TO REDUCE THE AREA IN THE STATE SPACE, plot X.s and Xo to make sure traps have the same scale ## 

#define statespace, using omineca subpopulation data 
...
cam_sites
#J.s <- 25
J.s<-320 # placing 1761 hair traps on a 60 grids of 12km

#J.o <- 50
J.o<-33 # placing 158 camera traps on 44 grids of 12km 

...
init_simple <- function() {
  ...
  zi[1: (20 *(dim(y)[1])), ] <- 1 # deleted 5* in 1:5*(dim(y)[1]))
...
for (i in 1:nsims) {
  name.i <- paste("dat.columbian_", stub, "_", i, sep = "")
  obj.i <- get(name.i)
  out.i <- paste("open.columbian_", stub, "_", i, sep = "")
...
jags.View(open.columbian_test_IOM_1)
